...
app.get('/posts', async (req, res, next) => {
    ...
    const data = await db.Post.find(selection).populate('postedBy', 'name email -_id').sort({posted_at:-1})
    .catch(e => console.log({
...
app.get('/my_posts/:token',async(req,res,next)=>{
    ...
    const data = await db.Post.find({postedBy:userJwt.userId},selection).populate('postedBy', 'name email -_id').sort({posted_at:-1})
    .catch(e => console.log({
        title: "Error in grabbing data from db",
        error: e
    }))
...
app.get('/types',async(req,res,next)=>{
    let concrete = await db.Post.find({beach_type:"Concrete"}).count();
    let rocky = await db.Post.find({beach_type:"Rocky"}).count();
    let sandy = await db.Post.find({beach_type:"Sandy"}).count();

    let lifeguard_yes = await db.Post.find({lf_tower:"Yes"}).count();
    let lifeguard_no = await db.Post.find({lf_tower:"No"}).count();

    let pets_yes = await db.Post.find({pets_allowed_answer:"Yes"}).count();
    let pets_no = await db.Post.find({pets_allowed_answer:"No"}).count();

    let free_yes = await db.Post.find({free_beach:"Yes"}).count();
    let free_no = await db.Post.find({free_beach:"No"}).count();


    res.json({
        concrete_num:concrete,
        rocky_num:rocky,
        sandy_num:sandy,

        lifeguardY:lifeguard_yes,
        lifeguardN:lifeguard_no,
